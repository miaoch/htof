...
$(document).ready(function(){
	$("#box").tree({
        ...
        onDrop: function (target, source, point) {
        	var dest = $('#box').tree('getNode', target);
        	$.ajax({
                url:"changeGroup",
                data:{"sourceid":source.id, "groupid":dest.id},
            });
        }
    });
    $("#userdlg").dialog({
...
function removenode(){
	var node = $('#box').tree('find', nodeid);
	var child_node = $('#box').tree('getChildren', node.target);
	if (child_node.length > 0) {
		$.messager.alert('警告',"不能删除非空分组！");
		return false;
	}
...
function saveUserNode() {
	$("#fm").form("submit", {
		url : "saveOrUpdateUserNode?nodeid="+nodeid+"&id="+id,
		onSubmit : function() {
